...
class HForm extends \yii\base\Component {
    ...
    public function save() {
        $hasErrors = false;
        if ($this->primaryModel !== null) {
            foreach ($this->primaryModel as $key => $value) {
                $internlnam = ProfileField::find()->select(['field_type_class'])->where(['internal_name' => $key])->one();
                if (!empty($internlnam)) {
                    $fieldtype = explode('\\', $internlnam->field_type_class);
                    $fieldtypes = $fieldtype[5];
                    if ($fieldtypes == 'MultiSelectDropdown') { 
                        $to_implode = $this->primaryModel->$key; 
                        if (!empty($to_implode) && is_array($to_implode)) {
                            $store_value = implode(',', $to_implode);
                            $this->primaryModel->$key = $store_value;
                        } 
                    }
                }
            } 
            ...
            if (isset($definition['type'])) {
                switch ($definition['type']) {
                    ...
                    case 'markdown':
                        ...
                        return $returnField;
                    case 'tags':
                       $tag_field = $this->form->field($model, $name)->textInput(['data-role' => 'tagsinput']);
                        return $tag_field;
                        
                    case 'multiselect':
                        $data = $definition['items'];
                        $data_option = $data['option'];
                        $data_maxselectnlength = $data['maxselectnlength'];
                    
                        if(!empty($model->$name) && !is_array($model->$name)) {
                            $model->$name = explode(',', $model->$name);
                        }
                        return $this->form->field($model, $name)->widget(Select2::classname(), [
                            'data' => $data_option,
                            'language' => 'en',
                            'showToggleAll' => false,
                            'options' => ['placeholder' => 'Enter text to filter options below..', 'multiple' => true,'class'=>'hint-txt','data-maxlength'=>$data_maxselectnlength],
                            'pluginOptions' => [
                                'maximumSelectionLength' => $data_maxselectnlength,
                                'allowClear' => true,
                                'closeOnSelect' => false,
                            ],
                        ]);
